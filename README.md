...
- **Self-signed servers are opt-in per site, with TOFU cert pinning.** When "Allow self-signed" is on, the app pins the first leaf certificate it sees (SHA-256 fingerprint stored in Keychain) and rejects every subsequent certificate whose fingerprint doesn't match. There's no UI bypass once a cert is pinned. Legitimate rotation is handled by the **Re-pin Certificate** button in Settings, which reruns the TOFU handshake explicitly. Self-signed mode only persists together with a pinned fingerprint — if the server's certificate actually validates against the OS trust store, the site is saved as a regular fully-validated HTTPS site — and unpinned connections outside the explicit trust prompt fail closed (since 0.3.2).
